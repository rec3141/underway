        ...
        for stamp, path in chosen:
            try:
                ...
                if layout == "portrait":
                    ...
                    key = hashlib.sha256(json.dumps(["portrait-v4",key,[(p.stat().st_size,p.stat().st_mtime_ns) for p in dependencies]]).encode()).hexdigest()
                ...
                if not cached.is_file():
                    ...
                    if layout == "portrait":
                        ...
                        label = stamp.strftime("%Y-%m-%d %H:%M UTC")
                        left, top, right, bottom = draw.textbbox((0,0),label,font=font)
                        text_width, text_height = right-left, bottom-top
                        padding = max(4,width//90)
                        x = (width-text_width)//2
                        y = canvas.height-width//24-padding-text_height
                        draw.rectangle((x-padding,y-padding,x+text_width+padding,y+text_height+padding),fill="black")
                        draw.text((x-left,y-top),label,fill="white",font=font)
                    else:
