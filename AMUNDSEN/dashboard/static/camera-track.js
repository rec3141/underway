...
(() => {
  ...
  UW.fetchJSON('data/camera-track.json').then(payload=>{
    ...
    const popup=document.createElement('div');popup.id='photopop';popup.className='campop';popup.hidden=true;popup.style.width='min(480px, 85%)';popup.setAttribute('role','dialog');popup.setAttribute('aria-label',ui("Camera photo"));
    const head=document.createElement('div');head.className='head';const heading=document.createElement('b'),close=document.createElement('button');close.textContent='✕';close.title=ui("Close photo");close.setAttribute('aria-label',ui("Close photo"));const spacer=document.createElement('span');head.append(heading,spacer,close);
    ...
    const foot=document.createElement('div');foot.className='foot';const link=document.createElement('a');link.textContent=ui("Open full-size photo");link.target='_blank';link.rel='noopener';const roiNote=document.createElement('span');roiNote.style.marginLeft='10px';foot.append(link,roiNote);popup.append(head,imageBox,foot);document.querySelector('#map').parentElement.append(popup);
    img.onload=()=>{const supported=img.naturalWidth===3648&&img.naturalHeight===2052;roi.style.display=supported?'block':'none';roiNote.textContent=supported?ui("Orange outline: analyzed ROI"):ui("ROI unavailable for these image dimensions");};
    ...
    img.onerror=()=>{roi.style.display='none';roiNote.textContent='';heading.textContent=ui("Photo unavailable — archive may be offline");};
    window.addEventListener('uw:localechange',()=>{
      popup.setAttribute('aria-label',ui('Camera photo'));
      close.title=ui('Close photo');close.setAttribute('aria-label',ui('Close photo'));
      link.textContent=ui('Open full-size photo');
      if(!popup.hidden&&img.complete){if(img.naturalWidth)img.onload();else img.onerror();}
    });
    ...
    UW.registerPanel('Camera · feature PC1 · mean RGB',{group:'Bridge',
      ...
      render(el,plot){
        const selectedPC=modes.indexOf(UW.state.colour),k=Math.max(0,selectedPC),axisLabel=selectedPC>=0?`${ui(names[k])} (PC${k+1})`:ui("Mean RGB (0–255)");
        const yValue=p=>selectedPC>=0?score(p,k):p.rgb.reduce((a,b)=>a+b,0)/3;
        el.querySelector('h3').textContent=ui(selectedPC>=0?modes[k]:RGB);
        el.querySelector('h3').title=ui("Click to cycle Texture → Brightness → Colorfulness → Patchiness → mean RGB. Names are provisional PC interpretations.");
        ...
        if(!points.length){if(plot.data)Plotly.purge(plot);plot.className='plot empty';plot.textContent=rows.length?ui("No matching underway distances in this span"):ui("No camera data in this span for the selected legs");return;}
        ...
        UW.reactPlot(plot,[trace],{...UW.THEME,margin:{l:52,r:8,t:6,b:34},showlegend:false,dragmode:'pan',xaxis:{...UW.THEME.xaxis,_uwIdentity:UW.state.xmode,type:UW.state.xmode==='time'?'date':'linear',range:sharedRange,title:{text:UW.state.xmode==='time'?'UTC':window.UWI18n.variable('Distance travelled (km)')}},yaxis:{...UW.THEME.yaxis,_uwIdentity:selectedPC>=0?modes[k]:RGB,title:{text:ui(axisLabel)}}},UW.CFG).then(()=>{UW.axisZoom(plot);UW.linkX(plot);plot.removeAllListeners?.('plotly_click');plot.on('plotly_click',ev=>{if(cycle.includes(UW.state.colour))openPhoto(points[ev.points?.[0]?.pointIndex]?.p);});});
      }
    });
    ...
    setInterval(async()=>{if(refreshing||document.hidden)return;refreshing=true;
      ...
      }catch(error){console.warn(ui("Keeping last good camera data"),error);}finally{refreshing=false;}
    },60000);
  }).catch(error=>console.warn(ui("Camera track unavailable"),error));
})();
