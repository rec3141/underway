...
(() => {
  ...
  function render(mode,el,plot){
    ...
    const fz=U.fz||((n)=>n);
    U.reactPlot(plot,traces,{...U.THEME,margin:{l:fz(mode===2?105:52),r:8,t:fz(6),b:fz(34)},barmode:'stack',showlegend:false,hovermode:'closest',hoverdistance:14,xaxis:{...U.THEME.xaxis,type:U.state.xmode==='time'?'date':'linear',range,autorange:false,title:{text:U.state.xmode==='time'?`ship time (${U.tzAbbr?.()||'local'})`:'distance along track (km)',font:{size:fz(12)},standoff:4},tickfont:{size:fz(12)},hoverformat:U.state.xmode==='time'?'%Y-%m-%d %H:%M:%S':'.1f',ticksuffix:U.state.xmode==='time'?'':' km',nticks:Math.max(2,Math.floor((plot.clientWidth||300)/fz(100))),tickangle:0,automargin:true},yaxis:{...U.THEME.yaxis,range:mode===2?undefined:[0,100],title:{text:mode===2?'':'%',font:{size:fz(12)},standoff:2},tickfont:{size:fz(12)}}},U.CFG).then(()=>{U.axisZoom(plot);U.linkX(plot);plot.removeAllListeners?.('plotly_afterplot');plot.on('plotly_afterplot',()=>pointAt(cursor));selectedCursor();plot.removeAllListeners?.('plotly_click');plot.on('plotly_click',e=>{const p=e.points?.[0]?.customdata;if(p?.id){choose(p)}})});
  }
