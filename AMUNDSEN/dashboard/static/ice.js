...
(() => {
  ...
  function chip(mode){const p=latest();if(p&&mode===2){const [name,keys]=compactSurface.reduce((a,b)=>(groupedSurface(p,b[1])??-1)>(groupedSurface(p,a[1])??-1)?b:a);return {text:`${ui(name)} · ${groupedSurface(p,keys)??0}%`,image:null}}return {text:p?(mode===1?`${ui(p.ice===0?'water':types[dominant(p)]||'ice')} · ${p.ice}%`:`${p.ice}%`):'—',image:p&&mode>=3?image(p,'roi'):null}}
  ...
  const popup=document.createElement('dialog');popup.className='ice-preview';popup.innerHTML=`<div class="head"><button data-step="-1" aria-label="${uh("Previous image")}">←</button><button data-step="1" aria-label="${uh("Next image")}">→</button><button class="close">${uh("Close ✕")}</button></div><p></p><img alt="${uh("Ship camera context with ROI outline")}">`;document.body.append(popup);popup.querySelector('.close').onclick=()=>popup.close();
  function photoCaption(p){return `${U.fmtTs(p.time)} · ${ui(p.status)} · ${p.ice==null?ui('pending'):ui('{percent}% ice',{percent:p.ice})}`;}
  function choose(p){if(!p)return;recenterPhoto(p);chosen=p;popup.querySelector('p').textContent=photoCaption(p);popup.querySelector('img').src=image(p,'source');U.refreshExtraData();selectedCursor();if(!popup.open)document.querySelector('.ice-roi')?.focus({preventScroll:true})}
  window.addEventListener('uw:localechange',()=>{
    popup.querySelector('[data-step="-1"]').setAttribute('aria-label',ui('Previous image'));
    popup.querySelector('[data-step="1"]').setAttribute('aria-label',ui('Next image'));
    popup.querySelector('.close').textContent=ui('Close ✕');
    popup.querySelector('img').alt=ui('Ship camera context with ROI outline');
    if(chosen)popup.querySelector('p').textContent=photoCaption(chosen);
  });
  ...
  U.registerColour({name:colour,resolved:true,unit:ui("ice type"),rgb:true,sizes:d=>matches(d).map(p=>p?.ice==null?3:4+8*p.ice/100),onPoint:(d,i)=>choose(matches(d)[i]),values:d=>matches(d).map(p=>!p||p.ice==null?'#000000':p.ice===0?'#8b9bb0':palette[dominant(p)])});
  ...
  function meanTraces(base,meta,values,valid,c){
    ...
    for(let i=1;i<meta.length;i++){
      ...
      if(!groups.has(key))groups.set(key,{...base,name:ui("1 h centered mean"),mode:'lines',x:[],y:[],customdata:[],line:{color,width:3},hovertemplate:'%{y:.1f}% · 1 h centered mean<br>%{x}<extra></extra>'});
  ...
  function render(mode,el,plot){
    ...
    const xx=expanded.map(r=>U.state.xmode==='time'?U.plotDate(+r.x):r.x),meta=expanded.map(r=>r.p),trace=(y,name,color)=>({type:'scatter',mode:'lines+markers',x:xx,y,name:ui(name),connectgaps:false,customdata:meta,marker:{size:3,color},line:{color,width:2},hovertemplate:'%{y}%<br>%{customdata.status}<extra>'+ui(name)+'</extra>'});
    const surface=[...(el.classList.contains('wide')?detailedSurface:compactSurface)].reverse();
    let traces=mode===4?[{type:'scatter',mode:'markers',x:points.map(r=>U.state.xmode==='time'?U.plotDate(+r.x):r.x),y:points.map(()=>50),marker:{opacity:0},hoverinfo:'skip'}]:mode===0?concentrationTraces(trace(meta.map(p=>p.ice),ui("Total ice"),'#5cc8ff'),meta):mode===1?typeOrder.map(k=>({...trace(meta.map(p=>p.types?.[k]??null),types[k],palette[k]),type:'bar',width:U.state.xmode==='time'?120000:undefined})):[{type:'heatmap',x:xx,y:surface.map(([name])=>ui(name)),z:surface.map(([,keys])=>meta.map(p=>groupedSurface(p,keys))),zmin:0,zmax:100,colorscale:'Viridis',customdata:surface.map(()=>meta),hovertemplate:'%{y}: %{z}%<extra></extra>',hoverongaps:false,showscale:false}];
