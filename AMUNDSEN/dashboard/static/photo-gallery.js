...
(() => {
  ...
  function slideshow({items, id, onSelect}) {
    ...
    const show = delta => {
      ...
      if (!item) return;
      current = item.id;
      const img = dialog.querySelector('img'); img.src = item.src; img.alt = item.alt || item.title || 'Photograph';
    ...
    dialog.onclose = () => { dialog.remove(); if (current !== id) onSelect(current); };
  ...
  function wireDetail(figure, options) {
    ...
    figure.innerHTML = `<nav class="photo-navigation" aria-label="Gallery navigation"><button type="button" class="photo-gallery">← Gallery</button><span>${items.indexOf(item) + 1} / ${items.length}</span><button type="button" class="photo-prev" aria-label="Previous photo" ${items.length < 2 ? 'disabled' : ''}>‹</button><button type="button" class="photo-next" aria-label="Next photo" ${items.length < 2 ? 'disabled' : ''}>›</button></nav><button type="button" class="photo-open" aria-label="Open fullscreen slideshow"><img src="${esc(item.src)}" alt="${esc(item.alt || item.title || 'Photograph')}"></button><figcaption>${item.credit ? esc(item.credit) + ' · ' : ''}click photo for fullscreen</figcaption>`;
