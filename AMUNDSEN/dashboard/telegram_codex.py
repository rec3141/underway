"""Explicit operator-only Telegram commands backed by durable Codex sessions."""
...
USAGE = ("Use /codex <session> <message>, or reply to a text message with "
         "/codex <session>. Repeat the same session name to resume it. "
         "Names use 1-64 letters, numbers, underscores or hyphens.")
...
def connect():
    ...
    db = sqlite3.connect(DB_DIR / 'telegram_codex.sqlite', timeout=10)
...
def submit(msg, update_id, tg):
    """Authorize before accepting text; never route ordinary messages implicitly."""
    ...
    parts = (msg.get('text') or '').strip().split(None, 2)
    if len(parts) < 2 or not re.fullmatch(r'[A-Za-z0-9][A-Za-z0-9_-]{0,63}', parts[1]):
        return USAGE
    name = parts[1]
    ...
    with connect() as db:
        if db.execute('SELECT 1 FROM jobs WHERE id=?', (update_id,)).fetchone():
            return f'Codex [{name}]: this message was already queued.'
    ...
    start(tg)
    return f'Codex [{name}]: queued. I will reply here when it finishes.'


def command(thread=None):
    binary = os.environ.get('UNDERWAY_CODEX_BIN') or shutil.which('codex') or str(Path.home() / '.local/bin/codex')
    args = [binary, 'exec', '-s', 'workspace-write', '-c', 'approval_policy="never"']
    if thread:
        args += ['resume', thread]
    return args + ['--json', '-']
...
def execute(job):
    ...
    cwd = Path(os.environ.get('UNDERWAY_CODEX_CWD') or Path(__file__).resolve().parents[2])
    ...
    if failed or proc.returncode:
        return (answer + '\n\n' if answer else '') + 'Codex could not finish this turn. The saved session can be resumed with another /codex message. Check the Telegram service journal for setup issues.'
    return answer or 'Codex finished without a text response.'


def finish(db, job, answer):
    prefix = f"Codex [{job['name']}]\n"
...
def work(tg):
    DB_DIR.mkdir(parents=True, exist_ok=True)
    with (DB_DIR / 'telegram_codex.lock').open('a') as lock:
        try:
        ...
        with connect() as db:
            for job in db.execute("SELECT * FROM jobs WHERE status='running'").fetchall():
                finish(db, job, 'The bot restarted during this turn. It was not replayed automatically. Send /codex with the same session name to continue.')
        while True:
            try:
                with connect() as db:
                ...
                if job:
                    try:
                        ...
                        answer = 'Codex could not finish this turn. Check its login, workspace permissions and service journal, then resume with /codex and the same session name.'
                    with connect() as db:
