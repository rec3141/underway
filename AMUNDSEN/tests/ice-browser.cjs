...
(async()=>{try{
 ...
 const mean=`${concentration}.filter(t=>t.name==='1 h centered mean')`;
 assert.deepEqual(await evaluate(`${mean}[0].y`),[30,30,null]);
 const initialMeanColour=await evaluate(`${mean}[0].line.color`);
 assert.equal(await evaluate(`${concentration}.slice(0,2).every(t=>t.mode==='markers'&&t.marker.size===6)`),true);
 ...
 assert.notEqual(await evaluate(`${mean}[0].line.color`),initialMeanColour);
 assert.equal(await evaluate(`${mean}[0].line.color`),await evaluate('UW.colourAt(UW.cmap("Viridis",true),3/63)'));
 ...
 assert.equal(await evaluate(`${mean}[0].line.color`),'rgb(12,34,56)');
 assert.deepEqual(await evaluate(`${mean}[0].y`),[30,30,null]);
 ...
 assert.equal(await evaluate(`${mean}[0].line.color`),'#7d8895');
 ...
 console.log('PASS selected numeric/RGB colours, reversed limits, missing/low-flow gray points, larger observation dots and colored running mean');
