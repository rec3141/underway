...
const seaSavePorts='10.0.0.22:'+Array.from({length:9},(_,i)=>49160+i).join(',');
const liveData={port:5555,tcp:seaSavePorts,active:'10.0.0.22:49161',tcp_state:'probing',columns:['scan','depth','temperature'],pressure_col:'depth',current:liveCast('depth',25),last:liveCast('depth_m',10)};
const pageViews=[];
...
(async()=>{
  try {
    ...
    assert.equal(await evaluate('document.querySelector("#livecfgform [name=tcp]").value'),seaSavePorts);
    assert.equal(await evaluate('document.querySelector("#livesetupstatus").textContent.includes("PROBING")'),true);
    await evaluate('document.querySelector("#livecfgform [name=tcp]").value="10.0.0.99:1234";document.querySelector("#livecfgform [data-seasave-range]").click()');
    assert.equal(await evaluate('document.querySelector("#livecfgform [name=tcp]").value'),'10.0.0.99:'+Array.from({length:9},(_,i)=>49160+i).join(','));
