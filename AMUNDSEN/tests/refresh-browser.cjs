...
(async()=>{
  try {
    ...
    if(process.env.WIKI_FEEDBACK_UI){
      ...
      await until('!document.querySelector(".wiki-image-preview").open');
      await evaluate('UW.onHistoryClick("place-artifact",{lat:76,lon:-78})');
    ...
    if(process.env.UNDERWAY_UI) {
      ...
      await until('window.UW?.state.data?.vars["Bottom depth (m)"] && document.querySelector("#aggtable tbody tr")');
      assert.equal(await evaluate('document.querySelector("#underway-table").open'),false);
