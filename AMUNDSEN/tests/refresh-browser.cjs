...
function dataset(p) {
  ...
  if(p.startsWith('/data/w-')) return {label:p.includes('3h')?'3h':'1h',step_s:10,n:2,t:[t,t+10000],lat:[76,76.001],lon:[-78,-78.001],dist_km:[0,1],leg:[0,0],tsg_low:[false,true],vars:{'SST (°C)':[generation,generation]},limits:{'SST (°C)':[0,10]},start:new Date(t).toISOString(),end:new Date(t+10000).toISOString()};
...
(async()=>{
  try {
    ...
    await evaluate('window.UW.state.raw.tsg_low=[false,true];window.UW.showTab("underway");window.UW.renderMap()');
    await until('document.querySelector(".panel .plot")?.data?.length===2');
    assert.equal(await evaluate('document.querySelector(".panel .plot").data[0].y[1]'),null);
    assert.equal(await evaluate('document.querySelector(".panel .plot").data[1].marker.color'),'#858b93');
    assert.equal(await evaluate('document.querySelector(".panel .plot").data[1].y[1]'),6);
    await until('document.querySelector("#map").data?.some(t=>t.name==="track · low intake flow")');
    await evaluate('window.UW.showTab("calendar")');
