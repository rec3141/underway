...
class ChangesTests(unittest.TestCase):
    ...
    def test_changes_only_subscriber(self):
        ...
        self.assertEqual(msgs["42"], [f"Schedule changes: 1 added, 0 removed, 0 edited — {alerts.CHANGE_URL}"])   # no heads-up, no start, no completion
    ...
    def test_multiple_changes_produce_one_short_notice(self):
        ...
        rows = [_row(f"S{i}", "CTD", self.t(60 + i * 20), self.t(120 + i * 20)) for i in range(5)]
        events = [("added", rows[0], "Added detailed operation"),
                  ("added", rows[1], "Added detailed operation"),
                  ("removed", rows[2], "Removed detailed operation"),
                  ("moved", rows[3], "Moved detailed operation"),
                  ("plan_changed", dict(rows[4], change_at=self.now.isoformat()), "Edited detailed operation")]
        self.assertEqual(alerts.messages_for([sub], events, state, self.now),
                         [(sub, [f"Schedule changes: 2 added, 1 removed, 2 edited — {alerts.CHANGE_URL}"])])
        self.assertEqual(alerts.messages_for([sub], events, state, self.now), [])
    ...
    def test_future_end_time_and_small_start_edits_notify(self):
        ...
        for changed in (_row('S1', 'CTD', self.t(60), self.t(125)),
            ...
            self.assertEqual(alerts.messages_for([sub], events, state, self.now),
                             [(sub, [f"Schedule changes: 0 added, 0 removed, 1 edited — {alerts.CHANGE_URL}"])])
    ...
    def test_telegram_change_notice_has_no_heading_or_bullets(self):
        ...
        self.assertEqual(tg.sent, [('42', f'Schedule changes: 1 added, 0 removed, 0 edited — {alerts.CHANGE_URL}')])
