...
(async()=>{
  try {
    ...
    if(process.env.I18N_UI){
      ...
      assert.equal(await evaluate('JSON.stringify({win:UW.state.win,colour:UW.state.colour,hidden:[...UW.state.hidden],data:UW.state.raw.vars})'),selectionBefore);
      assert.equal(await evaluate('document.querySelector("#mapexport").getAttribute("aria-label")'),'Exporter la carte');
      ...
      await evaluate('UW.showTab("underway");const chip=document.getElementById("c-SST_C_");if(chip.getAttribute("aria-pressed")!=="true")chip.click()');
      await evaluate('document.querySelector("#p-SST_C_").scrollIntoView()');
      await until('!!document.querySelector("#p-SST_C_ .plot")?._fullLayout');
      console.log('PASS localized source plot is ready');
      await evaluate('Plotly.relayout(document.querySelector("#p-SST_C_ .plot"), {"yaxis.range":[0.25,2.75],"yaxis.autorange":false})');
      const plotBefore=await evaluate('JSON.stringify(document.querySelector("#p-SST_C_ .plot").data.map(t=>t.y))');
      await evaluate('document.querySelector("#p-SST_C_ .plot-export").click()');
      await until('!!document.querySelector(".export-canvas")?.data && !document.querySelector("[data-format=png]").disabled');
      console.log('PASS localized export preview is ready');
      assert.equal(await evaluate('document.querySelector("[data-format=png]").textContent'),'Exporter en PNG');
      await evaluate('const input=document.querySelector(".plot-export-dialog [name=width]");input.value=640;input.dispatchEvent(new Event("change"))');
      await wait(100);
      await evaluate('UWI18n.setLocale("en")');
      assert.equal(await evaluate('document.querySelector(".plot-export-dialog [name=width]").value'),'640');
      assert.equal(await evaluate('document.querySelector("[data-format=png]").textContent'),'Export PNG');
      assert.equal(await evaluate('JSON.stringify(document.querySelector("#p-SST_C_ .plot").data.map(t=>t.y))'),plotBefore);
      assert.deepEqual(await evaluate('document.querySelector("#p-SST_C_ .plot")._fullLayout.yaxis.range'),[0.25,2.75]);
      await evaluate('document.querySelector(".export-close").click();UWI18n.setLocale("fr-CA")');
