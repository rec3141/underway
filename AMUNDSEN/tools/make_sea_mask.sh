#!/usr/bin/env bash
# Build the water mask the sea-route estimate (dashboard/searoute.py) walks:
# a lon/lat grid over a box, one byte per cell, 1 where the cell holds water.
#
...
# out, which is the right side to err on for a distance estimate. Needs GDAL
# (gdalbuildvrt, gdalwarp) and numpy.
set -euo pipefail
...
gdalwarp -q -te "${BBOX[@]}" -tr "$DLON" "$DLAT" -r min -ot Int16 -of ENVI -multi -wo NUM_THREADS=ALL_CPUS \
    "$WORK/global.vrt" "$WORK/min.bil"
python3 - "$WORK/min.bil" "$OUT" "${BBOX[@]}" "$DLON" "$DLAT" <<'PY'
import re, sys
import numpy as np
bil, out, xmin, ymin, xmax, ymax, dlon, dlat = sys.argv[1], sys.argv[2], *map(float, sys.argv[3:])
hdr = open(bil[:-4] + ".hdr").read()
ncol, nrow = (int(re.search(rf"{k}\s*=\s*(\d+)", hdr).group(1)) for k in ("samples", "lines"))
order = ">" if re.search(r"byte order\s*=\s*1", hdr) else "<"
elev = np.fromfile(bil, dtype=order + "i2").reshape(nrow, ncol)
water = (elev < 0)[::-1]                         # row 0 at the south edge, like the lat axis
np.savez_compressed(out, water=np.packbits(water, axis=1), shape=np.array(water.shape),
